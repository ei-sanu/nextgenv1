...
export function AuthModal({ isOpen, onClose, onSuccess }: AuthModalProps) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.data.message === "2FA_REQUIRED") {
        setMode("otp");
        if (res.data.offline)
          setError("TRANSMISSION_OFFLINE: Use code '7777'.");
        return;
      }
  ...
  const handleResendOTP = async () => {
    ...
    try {
      const type =
        mode === "reset" ? "RESET" : mode === "otp" ? "2FA" : "VERIFICATION";
            ...
            <div className="relative hidden md:flex w-1/2 overflow-hidden border-r border-white/5">
              ...
              <div className="relative z-10 p-12 mt-auto">
                ...
                <p className="text-white/40 text-sm leading-relaxed max-w-[280px]">
                  Cryptographically secure multi-factor authentication for every
                  mission.
                </p>
              </div>
            </div>
              ...
              <div className="mb-10">
                <div className="flex items-center gap-2 mb-2">
                  {mode !== "login" && mode !== "signup" && (
                    <button
                      onClick={() =>
                        resetState(mode === "reset" ? "forgot" : "login")
                      }
                      className="p-1 -ml-1 text-white/40 hover:text-cyber transition-colors"
                    >
                ...
                <p className="text-white/40 text-sm">
                  {mode === "otp"
                    ? "MFA Protocol Initiated."
                    : mode === "login"
                      ? "Please enter your operator credentials."
                      : mode === "signup"
                        ? "Initialise your security profile."
                        : "Initialise credential recovery."}
                </p>
              </div>
                ...
                {mode === "forgot" && (
                  <div className="space-y-4">
                    <p className="text-white/40 text-[11px] leading-relaxed italic">
                      Note: Recovery requires access to your registered
                      mission-critical terminal (email).
                    </p>
                    <button
                      type="button"
                    ...
                    >
                      <KeyRound className="h-3 w-3" />I remember my password
                    </button>
                  </div>
                )}
                ...
                {mode === "reset" && (
                  <div className="space-y-6">
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] font-bold uppercase tracking-[0.2em] text-white/30 ml-1 text-center block">
                        Recovery Code
                      </label>
                ...
                {mode === "otp" && (
                  <div className="space-y-6 text-center">
                    <p className="text-white/40 text-sm leading-relaxed">
                      ...
                      <span className="text-cyber font-bold">{email}</span>.
                    </p>
                    <div className="flex justify-center">
                      <OTPInput value={otp} onChange={setOtp} />
                    </div>
