...
function NewScan() {
  ...
  const [scanStatus, setScanStatus] = useState<"idle" | "pending" | "running" | "completed" | "failed">("idle");
  const [scanProgress, setScanProgress] = useState(0);
  const [summary, setSummary] = useState<any>(null);
  const [vulnerabilities, setVulnerabilities] = useState<any[]>([]);
  const [scanLogs, setScanLogs] = useState<Array<{ timestamp: string; level: string; message: string }>>([]);
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        setScanLogs((logsData?.data || []).map((log: any) => ({
        ...
        if (!cancelled) {
          setErrorMessage((err as any)?.message || "Failed to refresh scan stream");
        }
      }
    };
  ...
  const statusClass = useMemo(() => {
    if (scanStatus === "completed") return "text-emerald-300 border-emerald-300/40 bg-emerald-500/10";
    if (scanStatus === "failed") return "text-red-300 border-red-300/40 bg-red-500/10";
        ...
        <section className="mt-8 rounded-2xl border border-white/10 bg-black/40 overflow-hidden">
          <div className="px-5 py-4 border-b border-white/10 flex items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              ...
              <p className="font-mono text-xs uppercase tracking-widest text-white/70">Live Analysis Stream</p>
            </div>
            <span className={`px-2.5 py-1 rounded-full border text-[10px] uppercase tracking-widest font-mono ${statusClass}`}>
              {scanStatus}
            </span>
          </div>
          ...
          <div className="max-h-72 overflow-y-auto border-y border-white/10 bg-gradient-to-b from-slate-950/80 to-black">
            {scanLogs.slice(-120).map((log, index) => (
              ...
              >
                <span className="text-white/35 mr-2">{new Date(log.timestamp).toLocaleTimeString()}</span>
                <span className={log.level === "error" ? "text-red-300" : log.level === "warn" ? "text-amber-300" : "text-cyan-200"}>
          ...
          <div className="p-5 space-y-3">
            ...
            {vulnerabilities.length === 0 && scanStatus !== "completed" && scanStatus !== "failed" && (
              <p className="text-sm text-white/50">Scanning in progress. Findings will stream here.</p>
            )}
            {vulnerabilities.length === 0 && (scanStatus === "completed" || scanStatus === "failed") && (
              <p className="text-sm text-white/50">No vulnerabilities detected for this run.</p>
            )}
            {vulnerabilities.map((vuln) => (
              <div key={vuln._id} className="rounded-xl border border-white/10 bg-white/[0.03] p-3">
                <div className="flex items-start justify-between gap-3">
                  <p className="font-medium">{vuln.title}</p>
                  <span className="text-[10px] font-mono uppercase tracking-widest text-white/55">{vuln.severity}</span>
                </div>
                <p className="mt-1 text-sm text-white/65">{vuln.description}</p>
              </div>
            ))}

            {summary && (
              <div className="rounded-xl border border-cyan-300/20 bg-cyan-500/5 p-3 text-sm">
                <p className="font-mono text-xs uppercase tracking-widest text-cyan-200/90">Result Summary</p>
                <p className="mt-1 text-white/80">{summary.message || "Scan completed."}</p>
                <p className="mt-1 text-white/60">Total Vulnerabilities: {summary.totalVulnerabilities ?? vulnerabilities.length}</p>
              </div>
            )}
          </div>
        </section>
