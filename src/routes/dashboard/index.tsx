...
function Dashboard() {
  ...
  if (loading) return <div className="p-10 text-white animate-pulse">Initializing Command Center...</div>;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-xl font-bold uppercase tracking-tight">Recent Scans</h2>
          </div>
            ...
            <table className="w-full text-left">
              ...
              <tbody className="text-sm font-medium">
                {recentScans?.map((scan: any) => (
                  <tr key={scan._id} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors">
                    <td className="px-6 py-4 font-mono truncate max-w-[200px]">{scan.target}</td>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <span className={`w-1.5 h-1.5 rounded-full ${scan.status === 'completed' ? 'bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.5)]' : 'bg-amber-400 animate-pulse'}`} />
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-xl font-bold uppercase tracking-tight">Live Intelligence</h2>
          </div>
          <div className="p-4 border border-white/10 rounded-2xl bg-black font-mono text-xs space-y-2 h-[400px] overflow-y-auto scrollbar-hide">
            <p className="text-emerald-400 opacity-50">[SYSTEM] Aegis Core v1.0.0 Online</p>
            ...
            {recentScans?.slice(0, 5).map((scan: any, i: number) => (
              <p key={i} className="text-white/40">
                &gt; {new Date(scan.startedAt).toLocaleTimeString()} - {scan.type.toUpperCase()} SCAN INITIALIZED FOR {scan.target}
              </p>
            ))}
          </div>
        </div>
...
function StatCard({ icon, label, value, subValue }: any) {
  return (
    <div className="p-5 border border-white/10 rounded-2xl bg-white/5 backdrop-blur-md hover:border-white/20 transition-all hover:translate-y-[-2px]">
      ...
      <div className="text-[10px] text-white/30 uppercase tracking-tighter">{subValue}</div>
    </div>
  );
}
