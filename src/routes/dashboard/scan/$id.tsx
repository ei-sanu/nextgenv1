...
function ScanDetail() {
  ...
  const handleExport = (format: 'pdf' | 'json') => {
    window.open(`http://localhost:5001/api/reports/${id}/export?format=${format}`, '_blank');
  };

  if (loading) return <div className="p-10 text-white animate-pulse">Decrypting Intelligence...</div>;
    ...
    <div className="p-6 space-y-8 max-w-7xl mx-auto text-white pb-20">
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div className="space-y-2">
          <div className="flex items-center gap-3">
            <h1 className="text-4xl font-bold tracking-tighter truncate max-w-[500px]">{scan?.target}</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {vulnerabilities.map((vuln) => (
          <div key={vuln._id} className="p-6 border border-white/10 rounded-3xl bg-white/[0.02] backdrop-blur-sm space-y-4 relative overflow-hidden group">
            <div className={`absolute top-0 right-0 w-32 h-32 blur-[60px] opacity-10 transition-opacity group-hover:opacity-20 ${getSeverityColor(vuln.severity)}`} />
            
            <div className="flex justify-between items-start">
              <span className={`px-2 py-0.5 rounded text-[10px] font-black uppercase ${getSeverityBadge(vuln.severity)}`}>
                {vuln.severity}
              </span>
              <div className="text-[10px] font-mono text-white/20">CVSS: {vuln.cvssScore}</div>
            </div>

            <h3 className="text-xl font-bold leading-tight">{vuln.title}</h3>
            <p className="text-white/50 text-xs line-clamp-3 leading-relaxed">{vuln.description}</p>

            <div className="pt-4 border-t border-white/5 space-y-4">
              <div className="space-y-1">
                <span className="text-[8px] uppercase tracking-widest font-mono text-white/20">Remediation Protocol</span>
                <p className="text-[10px] text-emerald-400 leading-normal">{vuln.remediation}</p>
              </div>
              {vuln.cve && (
        ...
        {vulnerabilities.length === 0 && (
          <div className="lg:col-span-3 flex flex-col items-center justify-center py-20 border-2 border-dashed border-white/5 rounded-3xl opacity-30">
            ...
            <p className="font-mono uppercase tracking-[0.2em] text-sm">No Vulnerabilities Detected</p>
          </div>
        )}
      </div>
    </div>
...
function getSeverityBadge(sev: string) {
  switch (sev.toLowerCase()) {
    ...
    case 'low': return 'bg-sky-500/10 text-sky-500 border border-sky-500/20';
