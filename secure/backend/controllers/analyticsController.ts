...
export const getOverviewStats = async (req: Request, res: Response) => {
  try {
    ...
    const criticalVulnerabilities = await Vulnerability.countDocuments({
      severity: "critical",
    });
    const highVulnerabilities = await Vulnerability.countDocuments({
      severity: "high",
    });
    const mediumVulnerabilities = await Vulnerability.countDocuments({
      severity: "medium",
    });
    const lowVulnerabilities = await Vulnerability.countDocuments({
      severity: "low",
    });
