  ...
  try {
    if (mongoose.connection.readyState !== 1) {
      return res.status(503).json({
        success: false,
        message:
          "Database is unavailable. Ensure MongoDB Atlas IP access is configured, then retry.",
      });
    }
    ...
    console.log(
      `[Controller] Scan created ${newScan._id} type=${value.type} target=${value.target}`,
    );
