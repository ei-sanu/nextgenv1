...
export const runWebScan = async (target: string, scanId: string) => {
  ...
  emitEvent("scan_progress", {
    scanId,
    progress: 10,
    message: `Starting Nuclei vulnerability scanning on ${target}`,
  });
  ...
  try {
    ...
    emitEvent("scan_progress", {
      scanId,
      progress: 80,
      message: `Nuclei scanning completed. Found ${nucleiResults.length} potential issues.`,
    });
    await logScanEvent(
      scanId,
      "info",
      `Nuclei completed. Candidate findings: ${nucleiResults.length}`,
    );
    ...
    for (const vuln of nucleiResults) {
      ...
      await logScanEvent(scanId, "warn", `Finding: ${newVuln.title}`, {
        severity: severityLower,
      });
    }
    ...
    emitEvent("scan_progress", {
      scanId,
      progress: 100,
      message: `Web scan finalization complete.`,
    });
