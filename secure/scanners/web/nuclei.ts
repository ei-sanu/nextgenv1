  ...
  try {
    ...
    if (/<script[\s>]/i.test(body)) {
      vulns.push({
        ...
        description: 'Page contains inline <script> tags which may increase XSS risk.',
        severity: 'low',
        evidence: body.slice(0, 500),
        remediation: 'Avoid inline scripts; use CSP and external scripts with integrity.'
      });
    }
  } catch (err: any) {
...
export const runNucleiScan = async (target: string): Promise<any[]> => {
  ...
  if (!available) {
    logger.warn(`Nuclei binary not found in PATH; falling back to basic HTTP checks for ${target}`);
