...
export const runNetworkScan = async (target: string, scanId: string) => {
  ...
  emitEvent("scan_progress", {
    scanId,
    progress: 10,
    message: `Starting Nmap port scanning on ${target}`,
  });
  ...
  try {
    ...
    emitEvent("scan_progress", {
      scanId,
      progress: 80,
      message: `Nmap scanning completed. Found ${nmapResults.length} potential issues.`,
    });
    await logScanEvent(
      scanId,
      "info",
      `Nmap completed. Candidate findings: ${nmapResults.length}`,
    );
    ...
    emitEvent("scan_progress", {
      scanId,
      progress: 100,
      message: `Network scan finalization complete.`,
    });
